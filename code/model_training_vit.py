...
for s in sets:
    ...
    x = layers.Lambda(lambda img: vit.preprocess_inputs(img))(x)
    
    x = base_model(x)

    outputs = layers.Dense(38, dtype = "float32")(x)
    ...
    fig.suptitle(
        f"Training and Validation Metrics — Vit-B/16 with {s}% of noise in data", fontsize=16
    )
    ...
    plt.savefig(
        f"output/plots/training_metrics_vitb16_{s}_new.png", dpi=300, bbox_inches="tight"
    )

    model.save(f"output/vit{s}/vitb16_{s}.keras")
