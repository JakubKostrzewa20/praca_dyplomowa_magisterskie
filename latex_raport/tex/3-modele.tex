...
\section{Modele}
W poniższym rozdziale przedstawiono rodziny modeli głębokiego uczenia, które zostały wykorzystane w pracy dyplomowej w ramach badania ich możliwości na wybranym wcześniej zbiorze danych zawierającym rośliny uprawe i choroby. Trzy z tych rodzin modeli reprezentują klasyczne podejście w postaci architektury CNN – ResNet, MobileNet oraz EfficientNet oraz jedena rodzina modeli bedący podstawową implementacją transformera wizyjnego – ViT. Wybór został podyktowany próbą rozwinięcia wcześniej dokonanych badań przedstawionych w przeglądzie literatury. Wybór szeroko używanych modeli CNN oraz nowego, mniej popularnego modelu architektury ViT pozwoli na porównanie nie tylko wydajności samych modeli, ale może dostarczyć ciekawych wniosków dotyczących typów architektur oraz ich skuteczności. Każdy z poniższych podrozdziałów przedstawia jedną z rodzin oraz krótko opisuję zasadę ich działania, która odróżnia go od innych.   
...
\end{figure}
\subsection{DenseNet}
DenseNet to rodzina modeli konwolucyjnych sieci neuronowych (CNN), stworzona przez naukowców z Uniwersytetu Kaliforniejskiego w Merced. Podobnie jak w modelach ResNet, zaimplementowano tutaj formę połączeń skórtowych między warstwami, jednak w różnej formię. Celem twórcóW było zaprojektowanie architektury, któa sprosta występującemu problemówi w czasach tworzenia - zanikającego gradientu. Twórcy tym samym dążyli do redukcji czasu klasyfikacji, aby model mógł być wprowadzony do użytku w ramach analizy wideo w czasie rzeczywistym. W DenseNet, każda warstwa otrzymuję wszystkie wyniki z poprzedzających ją warstw w postaci tensora. Dzięki temu model na każdej warstwie ma dostęp do całości danych, od surowych wejściowych, po te przetworzone przez kolejmne warstwy, a nie jak w normalnej sieci konwolucyjnej, czyli tylko wynik z poprzedzającej warstwy. Pozwala to na dodawanie nowych cech, bez tracenia wcześniej wyuczonych informacji.
